...
async function getTablePricePerHour(client, tableId) {
  const tableResult = await client.query(
    'SELECT price_per_hour FROM public.billiard_tables WHERE table_id = $1',
    [tableId]
  );

  if (tableResult.rowCount > 0 && tableResult.rows[0].price_per_hour != null) {
    return toNumber(tableResult.rows[0].price_per_hour, 50000);
  }

  return getCurrentPricePerHour(client);
}

module.exports = {
  getCurrentPricePerHour,
  getTablePricePerHour,
};
